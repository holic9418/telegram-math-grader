...
Zest 수학과 선생님들이 함께 쓰는 출석부예요. 여기 적으면 <b>공용 출석부 파일에 자동 기록</b>됩니다.
...
→ 이미 입력한 뒤에도 <b>특정 칸만 고쳐달라</b>고 하면 수정돼요. 예) <code>초5 7/15 남우현만 출석으로 바꿔줘</code>

<b>2) 출석 표기</b>
• 결석: <code>OO 결석</code> (사유 말하면 그대로, 없으면 '무단'으로 입력됨)
• 지각: <code>OO 30분 지각</code> / <code>병원가서 30분 늦음</code> → <code>30분(병원)</code>
• 조퇴: <code>휴가로 20분 조퇴</code> → <code>20분 조퇴(휴가)</code> (조퇴도 사유가 같이 들어가요)
• 숙제: 안 하면 <code>X</code>, 절반은 <code>50%</code>라고 입력됩니다. 사유 입력이 필요한 경우 안 가져왔으면 <code>미지참</code>, 안 받아갔으면 <code>미수령</code> 등으로 간략하게 알려주세요!
• 시험: 시험 이름과 학생·점수를 적으면 자동 입력돼요.
...
<b>5) 알림</b>
• 수업이 끝나고 <b>15분 뒤까지 그 반 출석이 입력되지 않으면</b> 담당 쌤에게 알림이 와요.
• 알림 시각을 바꾸려면 <code>알림 초5 21:00</code> 처럼 보내세요.
• <b>일정</b> : 반별 수업 요일 보기·변경 (예: <code>일정 고1 화목토</code>)
• 매일 밀린 미입력 출석도 한 번 더 알려드려요.
