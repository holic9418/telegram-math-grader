...
def admins_path():
    return os.path.join(DATA_DIR, "admins.json")


def load_admins():
    """{str(chat_id): {'name','status'}} — status: approved|pending. 하위 관리자."""
    p = admins_path()
    if os.path.exists(p):
        try:
            with open(p, encoding="utf-8") as f:
...
def is_admin(chat_id):
    """마스터이거나, 마스터가 승인한 관리자면 True (관리자 전용 기능 통과)."""
    if is_master(chat_id):
        return True
    a = load_admins().get(str(chat_id))
...

{
  ...
  "date": "M/D",
  "출석":   {"학생명": "O" 또는 "X(사유)"},
  "수업내용": "<문자열(반 전체 동일)>" 또는 {"A":"CLASS A 수업","B":"CLASS B 수업","나머지":"공통"},
  "과제수행": {"학생명": "O" 또는 "X"},
  "다음과제": "<문자열(반 전체 동일)>" 또는 {"학생명":"그 학생 숙제", "나머지":"나머지 공통 숙제"},
  "비고":   {"학생명": "<문자열 또는 점수>"},
...
- 수업내용: 기본은 문자열 하나(반 전체 동일).
    · 반이 CLASS A / CLASS B 로 나뉘어 그룹마다 다른 수업을 말하면 객체로 적는다: {"A":"A반 수업","B":"B반 수업"}.
      A반=1반=CLASS A, B반=2반=CLASS B 로 같은 뜻이다. 사용자가 '1반/2반'이라 해도 키는 "A"/"B" 또는 "1"/"2" 로 적으면 된다.
      예) '중3 A반은 원주각, B반은 원의 접선' → "수업내용":{"A":"원주각","B":"원의 접선"}.
      예) '중3 1반 삼각비, 2반 피타고라스' → "수업내용":{"1":"삼각비","2":"피타고라스"}.
      한 그룹만 다르고 나머지는 공통이면 {"A":"...","나머지":"공통 수업"} 형태도 된다.
    · 특정 학생만 다르면 {"학생명":"그 학생 수업","나머지":"공통"}도 가능. 그룹/학생 언급이 없으면 문자열로 둔다.
- 다음과제(다음 숙제): 기본은 문자열 하나로 반 전체 동일하게 적는다.
    · 특정 학생만 다른 숙제면 객체로 적는다: {"그 학생명":"그 학생 숙제", "나머지":"나머지 공통 숙제"}.
      예) '효담 숙제없음, 나머지는 프린트' → "다음과제":{"김효담":"없음","나머지":"프린트"}.
      '숙제없음/없음'이면 값은 "없음". '나머지/그 외/딴 애들'로 말한 것이 공통 숙제다.
    · 반이 CLASS A/B(=1반/2반)로 나뉘어 그룹마다 다른 숙제면 그룹 키로 적는다: {"A":"..","B":".."} 또는 {"1":"..","2":".."}.
      예) '1반은 보조교재 풀이, 2반은 연산프린트' → "다음과제":{"1":"보조교재 풀이","2":"연산프린트"}.
    · 특정 학생 언급 없이 하나면 그냥 문자열로 둔다.
...
• <b>휴강</b>: <code>7/20 휴강</code> · 사유 <code>8/17 휴강(대체공휴일)</code> · 기간 <code>8/18~8/22 휴강(학원방학)</code> · 취소 <code>8/17 휴강취소</code>
   ...
   → <b>반별</b>은 반을 앞에 붙여요: <code>중3 8/17 휴강</code> · <code>초5 8/18~8/22 휴강(개인사정)</code> · <code>중3 8/17 휴강취소</code>
...
• <b>칸 비우기</b>: <code>초5 7/15 남우현 출석 삭제</code> · <code>초5 8/5 수업내용 지워줘</code> · <code>오늘 고1 김서영 과제수행 취소</code> (그 칸만 비움 · 반은 학생으로 자동 추론)
...
async def cmd_timetable(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if arg:
        an = re.sub(r'반$', '', arg.replace(" ", ""))   # '초5반'·'초등zec반' → 반 떼기
...
async def cmd_clear(update: Update, context: ContextTypes.DEFAULT_TYPE, text):
    ...
    if any(k in text for k in ("다음과제", "다음숙제", "다음 숙제")):
        ...
        field = "출석"
    # 학생(수업내용 외) — 학생이 속한 반도 함께 얻는다
    student, stu_sheets = None, []
    if field != "수업내용":
        name, stu_sheets, amb = _find_student(text, latest)
        if amb:
            await update.message.reply_text("여러 명이에요: " + ", ".join(amb) + " — 성까지 붙여 주세요.")
            return
        student = name
    # 반 결정: 명시된 반 우선, 없으면 학생이 속한 반으로 추론
    cands = _match_preview_sheet(text, latest.sheetnames)
    if len(cands) > 1:
        await update.message.reply_text("어느 반인지 콕 집어주세요: " + ", ".join(cands))
        return
    if cands:
        sheet = cands[0]
    elif student and len(stu_sheets) == 1:
        sheet = stu_sheets[0]                       # 반을 안 적었어도 학생으로 추론
    elif student and len(stu_sheets) > 1:
        await update.message.reply_text(
            f"'{student}' 학생이 여러 반에 있어요: " + ", ".join(stu_sheets) + " — 반도 함께 알려주세요.")
        return
    else:
...
async def require_admin(update: Update) -> bool:
    """관리자(또는 마스터)면 True. 아니면 안내 후 False."""
    if is_admin(update.effective_chat.id):
        return True
    if get_master() is None:
        await update.message.reply_text(
            "아직 마스터가 없어요. 주인(원장)이 '마스터 등록' 이라고 보내 먼저 등록해 주세요."
        )
    else:
...
async def require_master(update: Update) -> bool:
    """마스터(최상위)면 True. 아니면 안내 후 False."""
    if is_master(update.effective_chat.id):
        return True
    await update.message.reply_text("🔒 이 기능은 마스터(최상위)만 쓸 수 있어요.")
    return False
...
async def _admin_status(update):
    ...
    if pend:
        lines.append("\n⏳ 관리자 신청 대기:")
        for k, nm in pend:
            lines.append(f"• {nm} (id {k}) → 승인: 관리자 승인 {k}")
    await update.message.reply_text("\n".join(lines))


async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """'마스터 등록'=최상위 등록(없을 때 첫 사람). '관리자 등록/신청'=관리자 신청(마스터 승인 필요)."""
    chat_id = update.effective_chat.id
    remember_chat(chat_id)
    low = deglue(update.message.text.lstrip("/").strip())
    master = get_master()
    if master is None:                      # 최초 부트스트랩 → 마스터
        set_master(chat_id)
        return await update.message.reply_text(MASTER_HELP, parse_mode="HTML")
    if is_master(chat_id):
        return await _admin_status(update)
    if low.startswith("마스터"):
        return await update.message.reply_text("이미 마스터가 있어요. 관리자는 '관리자 신청'으로 요청하세요.")
    if not any(k in low for k in ("등록", "신청")):
        return await update.message.reply_text(
            "관리자가 되려면 <b>관리자 신청</b> 이라고 보내세요. 마스터 승인 후 쓸 수 있어요.",
            parse_mode="HTML")
    # 관리자 신청
    admins = load_admins()
    cur = admins.get(str(chat_id))
    if cur and cur.get("status") == "approved":
        return await update.message.reply_text("이미 관리자예요. 👍")
...
async def cmd_admin_grant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """마스터: 관리자 신청 승인. 인자 없으면 현황."""
    if not await require_master(update):
        return
    admins = load_admins()
    if not context.args:
        return await _admin_status(update)
    key = _find_member_key(admins, " ".join(context.args))
    if not key:                              # 신청 안 했어도 멤버면 바로 관리자 지정 가능
        m = load_members()
        key = _find_member_key(m, " ".join(context.args))
        if key:
            admins[key] = {"name": m[key].get("name", "이름미상"), "status": "pending"}
    if not key:
        return await update.message.reply_text(
            "그런 신청자를 못 찾았어요. '관리자 승인'으로 목록을 확인하세요.")
    admins[key]["status"] = "approved"
    save_admins(admins)
    m = load_members()                       # 관리자는 사용 권한(멤버)도 자동 승인
    m.setdefault(key, {"name": admins[key]["name"]})["status"] = "approved"
    save_members(m)
    await update.message.reply_text(f"✅ {admins[key]['name']} 님을 관리자로 승인했어요.")
    try:
        await context.bot.send_message(
            int(key), "🛡 <b>관리자로 승인됐어요!</b> 이제 승인 관리·주간보고서·설정 기능을 쓸 수 있어요.",
            parse_mode="HTML")
    except Exception:
        pass


async def cmd_admin_revoke(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """마스터: 관리자 자격 박탈."""
    if not await require_master(update):
        return
    if not context.args:
        return await update.message.reply_text("누구 관리자 자격을 뺄지 이름/번호를 알려주세요. 예) 관리자 박탈 홍길동")
    admins = load_admins()
    key = _find_member_key(admins, " ".join(context.args))
    if not key:
        return await update.message.reply_text("그런 관리자를 못 찾았어요. '관리자 승인'으로 목록 확인.")
    name = admins[key].get("name", key)
    admins.pop(key, None)
    save_admins(admins)
    await update.message.reply_text(f"✅ {name} 님의 관리자 자격을 박탈했어요. (사용 권한은 유지)")
    try:
        await context.bot.send_message(int(key), "🛡 관리자 자격이 해제됐어요. (봇 사용은 계속 가능)")
    except Exception:
        pass


async def cmd_transfer_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """마스터: 마스터 자리를 다른 멤버에게 넘긴다. 예) '마스터 넘기기 홍길동'"""
    if not await require_master(update):
        return
    if not context.args:
        await update.message.reply_text(
...
def _split_class_dates(before, today):
    """'중3 8/17'·'중3 입시 8/18~8/22' → (반이름|None, [date,...]).
    날짜는 뒤쪽에 오고 앞은 반 이름. 날짜를 못 찾으면 (None, None)."""
    toks = before.split()
    for k in range(min(3, len(toks)), 0, -1):     # 긴 날짜부(공백 낀 범위)부터 시도
        ds = _holiday_dates(" ".join(toks[-k:]), today)
        if ds is not None:
            return (" ".join(toks[:-k]).strip() or None), ds
    return None, None


def parse_holiday_cmd(text, today):
    """휴강 명령 파싱. 반환 {'cancel','dates':[date,...],'reason','class'} 또는 None.
    지원: '8/17 휴강', '8/17일 휴강(대체공휴일)', '8/18~8/22 휴강(학원방학)',
          '7/20 휴강 폭우', '8/17 휴강취소', '오늘 휴강', '휴강 8/17',
          반별: '중3 8/17 휴강', '초5 8/18~8/22 휴강(개인사정)'.
    ...
    cls_hint = None
    if before:                                    # '중3 8/17 휴강' / '8/17 휴강'
        cls_hint, dates = _split_class_dates(before, today)
    else:                                         # '휴강 8/17' (동사 먼저 — 반 없음)
        dates = _holiday_dates(after, today)
    if dates is None:
        if (before or after) == "":               # 그냥 '휴강' → 도움말
            return {"cancel": cancel, "dates": [], "reason": None, "class": None}
        return None                               # 휴강은 있으나 날짜 형식이 아님
    return {"cancel": cancel, "dates": dates, "reason": reason, "class": cls_hint}


async def start_holiday(update: Update, context: ContextTypes.DEFAULT_TYPE, dates, reason=None, only=None):
    """주어진 날짜(들)에 수업이 있는 반을 휴강으로 지정 — 확인 후 적용.
    dates: date 객체 리스트(단일 또는 기간). only: 특정 시트명이면 그 반만."""
    chat_id = update.effective_chat.id
    wb, path = load_current_wb()
    if wb is None:
        await update.message.reply_text("이번 달 출석부가 아직 없어요.")
        return
    scope = [only] if only else wb.sheetnames
    ...
    who = f"{html.escape(only)} 은 " if only else ""
    if not targets:
        msg = (f"{who}<b>{span}</b> 은 이미 휴강이에요." if already
               else f"{who}<b>{span}</b> 에 수업이 없어요." if only
...
async def undo_holiday(update: Update, context: ContextTypes.DEFAULT_TYPE, dates, only=None):
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 승인된 멤버만 사용 가능 (마스터/관리자 등록·신청만 예외로 통과)
    _open_cmds = ("관리자등록", "관리자", "관리자설정", "마스터등록", "마스터",
                  "마스터설정", "관리자신청", "관리자신청하기")
    if low not in _open_cmds and not await gate_member(update, context):
        return
    ...
    # 휴강 지정·취소 — 단일 날짜 또는 기간(범위), 과목 전체 또는 특정 반
    #   전체: '8/17 휴강(대체공휴일)', '8/18~8/22 휴강(학원방학)', '오늘 휴강'
    #   반별: '중3 8/17 휴강', '초5 8/18~8/22 휴강(개인사정)', '중3 8/17 휴강취소'
    if "휴강" in text:
        ...
        if hol is not None:
            if not hol["dates"]:
                await update.message.reply_text(
                    ...
                    "기간 <code>8/18~8/22 휴강(학원방학)</code> · 반별 <code>중3 8/17 휴강</code> · "
                    "취소 <code>8/17 휴강취소</code>",
                    parse_mode="HTML")
                return
            only = None
            if hol["class"]:                        # 반을 지정했으면 그 반만
                wbp, _ = load_current_wb()
                sheets = wbp.sheetnames if wbp else []
                cands = _match_preview_sheet(apply_aliases(hol["class"]), sheets)
                if len(cands) == 1:
                    only = cands[0]
                elif not cands:
                    await update.message.reply_text(
                        f"'{html.escape(hol['class'])}' 반을 못 찾았어요. "
                        f"(반: {html.escape(', '.join(sheets))})")
                    return
                else:
                    await update.message.reply_text(
                        f"반이 여러 개예요: {html.escape(', '.join(cands))}\n"
                        "정확한 반 이름으로 다시 말해 주세요.")
                    return
            if hol["cancel"]:
                return await undo_holiday(update, context, hol["dates"], only)
            return await start_holiday(update, context, hol["dates"], hol["reason"], only)
    ...
    _m = re.match(r'^관리자\s*(?:승인|허가|수락)\s*(.*)$', low)
    if _m:                                     # '관리자 승인 홍길동'
        context.args = _m.group(1).split()
        return await cmd_admin_grant(update, context)
    _m = re.match(r'^관리자\s*(?:박탈|해제|거절|제거|삭제)\s*(.*)$', low)
    if _m:                                     # '관리자 박탈 홍길동'
        context.args = _m.group(1).split()
        return await cmd_admin_revoke(update, context)
    if low in ("마스터등록", "마스터", "마스터설정", "관리자등록", "관리자",
               "관리자설정", "관리자신청", "관리자신청하기"):
    ...
    # ── 칸 비우기: '초5 7/15 남우현 출석 삭제' · '오늘 고1 김서영 과제수행 취소' ──
    #   반이 맨 앞이 아니어도(오늘 …), '취소'로 끝나도 인식. 반 또는 항목이 있어야 함.
    _clear_end = re.search(r'(삭제|지워|지우|비워|없애|취소)\S*\s*$', low)
    _clear_ctx = re.search(r'(초|중|고)\d', low) or any(
        k in low for k in ("출석", "과제", "숙제", "수업내용", "수업", "비고", "다음과제"))
    if (_clear_end and _clear_ctx and _resolve_preview_date(low) is not None
            and "휴강" not in low
            and low not in ("실행취소", "방금취소", "되돌리기", "취소")):
    ...
    _dexpr = (r'(?:\d{1,2}\s*월\s*\d{1,2}\s*일?|\d{1,2}\s*월|\d{1,2}[/.]\d{1,2}'
              r'|(?:이번|저번|지난|지지난|다음)\s*주(?:\s*[월화수목금토일]요일)?'
              r'|이번달|지난달|저번달|오늘|어제|그제|그저께|엊그제|그끄제|내일|모레|글피'
              r'|[월화수목금토일]요일|\d+\s*일\s*(?:전|후))')
    _period = _dexpr + r'\s*(?:미리보기|보여줘|보여주세요|캡처|이미지로?)?'
    # 반: '중2' '초5A' '초3-1' '중3-2' '고2(미적분)' '초등zec' '중등zec' (+ 뒤에 '반' 붙어도)
    _cls = r'(?:(?:초|중|고)\d[a-zA-Z]*(?:-\d+)?(?:\([^)]*\))?|(?:초|중)등[a-zA-Z]+)반?'
    _stu = r'[가-힣]{2,4}'
