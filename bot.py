...
→ 이미 기록한 뒤에도 <b>특정 칸만 고쳐달라</b>고 말씀하시면 그 칸만 다시 기록해요. 예) <code>초5 7/15 남우현만 출석으로 바꿔줘</code>

<b>2) 이렇게 말씀하시면, 파일엔 이렇게 기록돼요</b>
• 결석: <code>OO 결석</code> 이라고 하시면 → 파일엔 <code>X (사유)</code> (사유 말씀하면 그대로, 없으면 <code>무단</code>)
• 지각: <code>OO 병원가서 30분 늦음</code> → 파일엔 <code>30분(병원)</code>
• 조퇴: <code>OO 휴가로 20분 조퇴</code> → 파일엔 <code>20분 조퇴(휴가)</code> (조퇴도 사유가 같이 기록돼요)
• 숙제: 안 했으면 <code>X</code>, 절반은 <code>50%</code>로 기록돼요. 사유가 필요하면 안 가져왔으면 <code>미지참</code>, 안 받아갔으면 <code>미수령</code> 처럼 간략히 말씀해 주세요!
...
<b>4) 내 담당 반 지정</b> (중요!)
• <b>담당 초5 중2</b> 처럼 보내주시면, 그 반 알림만 받으세요.

<b>5) 봇이 보내는 알림</b>
• 수업이 끝나고 <b>15분 뒤까지 그 반 출석을 안 알려주시면</b> 담당 쌤에게 알림을 보내드려요.
